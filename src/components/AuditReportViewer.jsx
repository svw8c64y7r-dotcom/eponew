...
export default function AuditReportViewer({ report }) {
  ...
  const safeScore = typeof overall_score === 'number' ? overall_score : 90;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel-glow p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center space-x-3 mb-1">
            ...
            <span className="text-xs text-cyber-muted font-mono">{formattedDate}</span>
          </div>
          ...
          <p className="text-xs text-cyber-muted mt-1">
            Execution Duration: <span className="text-cyber-cyan font-mono">{report.scan_duration_ms || 450} ms</span> (Vercel Serverless Posture Engine)
          </p>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right hidden sm:block">
            ...
            <div className="text-sm font-semibold text-white">
              {safeScore >= 90 ? 'Hardened Enterprise' : 'Action Required'}
            </div>
          </div>
          <div className={`w-20 h-20 rounded-2xl border-2 flex flex-col items-center justify-center font-mono ${getScoreColor(safeScore)}`}>
            <span className="text-3xl font-extrabold">{grade}</span>
            <span className="text-[10px] font-semibold tracking-wider opacity-80">{safeScore}/100</span>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm font-mono">
            ...
            <tbody className="divide-y divide-cyber-border/40">
              {(ports || []).map((item, idx) => (
                <tr key={`port-${item?.port ?? idx}-${idx}`} className="hover:bg-white/5 transition">
                  <td className="py-3 px-3 text-cyber-cyan font-bold">{item?.port ?? 'N/A'}</td>
                  <td className="py-3 px-3 text-white">{item?.service ?? 'Unknown'}</td>
                  <td className="py-3 px-3 text-cyber-muted">{item?.status ?? 'Protected'}</td>
        ...
        <div className="p-6 rounded-2xl bg-cyber-card border border-cyber-border">
          ...
          <div className="space-y-3 font-mono text-xs">
            {Object.entries(headers || {}).map(([key, item]) => (
              <div key={`hdr-${key}`} className="p-3 rounded-lg bg-cyber-bg/80 border border-cyber-border/60 flex justify-between items-start gap-2">
                <div>
                  <div className="font-semibold text-white">{key}</div>
                  <div className="text-[11px] text-cyber-muted mt-0.5 truncate max-w-[240px]">{item?.detail || 'Verified'}</div>
                </div>
      ...
      {/* Epotech Remediation Recommendations */}
      <div className="p-6 rounded-2xl bg-cyber-card border border-cyber-border">
        ...
        <ul className="space-y-2 text-xs text-cyber-muted font-mono">
          {(recommendations || []).map((rec, i) => (
            <li key={`rec-${i}`} className="flex items-start space-x-2 bg-cyber-bg/40 p-2.5 rounded-lg border border-cyber-border/30">
