...
import { runSecurityAudit } from '../lib/apiClient';
...
export default function DashboardPage({ user, onRequestModalOpen }) {
  ...
  const loadRequests = async () => {
    if (!user || !user.id) return; // CRITICAL: Ensure we have a logged-in user

    setLoadingRequests(true);
    try {
      // Direct Supabase query with Row Level filtering
      const { data, error } = await supabase
        .from('service_requests')
        .select('*')
        .eq('user_id', user.id) // ONLY fetch requests belonging to this client
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex border-b border-cyber-border space-x-4 font-mono text-xs">
        <button
          ...
          className={`pb-3 px-2 flex items-center space-x-2 transition border-b-2 ${activeTab === 'overview' ? 'border-cyber-cyan text-cyber-cyan font-bold' : 'border-transparent text-cyber-muted hover:text-white'
            }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-2 flex items-center space-x-2 transition border-b-2 ${activeTab === 'requests' ? 'border-cyber-cyan text-cyber-cyan font-bold' : 'border-transparent text-cyber-muted hover:text-white'
            }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-2 flex items-center space-x-2 transition border-b-2 ${activeTab === 'scanner' ? 'border-cyber-cyan text-cyber-cyan font-bold' : 'border-transparent text-cyber-muted hover:text-white'
            }`}
        >
                  ...
                  <div key={reqKey} className="p-6 rounded-2xl bg-cyber-card border border-cyber-border hover:border-cyber-cyan/30 transition flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      <div className="flex items-center space-x-3 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase ${req.priority === 'critical' ? 'bg-cyber-red/20 text-cyber-red' : 'bg-cyber-amber/20 text-cyber-amber'
                          }`}>
